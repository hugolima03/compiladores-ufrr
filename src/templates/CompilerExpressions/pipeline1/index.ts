...
export default class Pipeline1 {
  ...
  constructor(sourceCode: string) {
    ...
    this._weakPrecedenceParser = new WeakPrecedenceParser(PascalGrammar, '<program>', '$');
  }
  ...
  start() {
    ...
    const prods = this._weakPrecedenceParser.analisar(new TokensStack(parseLexemes, '$', this._sourceCode));

    // Step 2: Geração da árvore sintática
    const syntaxTree = Tree.parsearProducoes(prods, PascalGrammar);
